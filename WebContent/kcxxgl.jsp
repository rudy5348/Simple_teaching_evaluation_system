                        ...
                        <div class="layui-field-box">
                            <form class="layui-form">
                                <table class="layui-table">
                                        ...
                                        <tbody>
                                        <%
                                        List kc = (List)sess.getAttribute("kc");
                                        sess.removeAttribute("kc");
                                        if (kc.isEmpty() || kc.size() == 0 ){
                                        	%>
                                        	<tr>
	                                            <td colspan="3" style="text-align: center;">暂无课程内容</td>
	                                          </tr>
                                        	<%
                                        }else{
                                        	for(int i = 0 ; i < kc.size() ; i++){
                                        		Map map = (Map) kc.get(i);
                                        		%>
                                        		<tr>
		                                            <td><%=i+1 %></td>
		                                            <td><%=map.get("name") %></td>
		                                            <td><div class="layui-btn list_btn" data-type="<%=map.get("id") %>">删除</div><div class="layui-btn list_btn1" data-type="<%=map.get("id") %>">修改</div></td>
		                                          </tr>
                                        		<%
                                        	}
                                        }
            ...
            $('#add_classs').click(function(){
                layer.open({
                    ...
                    ,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">'+
                        '<input type="text" name="title" required  lay-verify="required" placeholder="请输入课程名" autocomplete="off" class="layui-input" id="kc_name">'+
                        '</div>'
                ...
                $.ajax({
                    ...
                    success:function(res){
                        layer.open({
                            ...
                            ,moveType: 1 //拖拽模式，0或者1
                            ,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">'+
                                '<input type="text" name="title" required  lay-verify="required" placeholder="请输入课程名" autocomplete="off" class="layui-input" id="kc_name" value="'+res['msg']+'">'+
                                '</div>'
                            ,yes: function(index, layero){
                                var kc_name =  $("#kc_name").val();
                                if (kc_name != ''){
