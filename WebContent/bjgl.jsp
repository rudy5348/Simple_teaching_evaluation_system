                        ...
                        <div class="layui-field-box">
                            <form class="layui-form">
                                <table class="layui-table">
                                        ...
                                        sess.removeAttribute("cl_list");
                                        if (cl_list.isEmpty() || cl_list.size() == 0 ){
                                        	%>
                                        	<tr>
	                                            <td colspan="4" style="text-align: center;">暂无班级数据</td>
	                                          </tr>
                                        	<%
                                        }else{
                                        	for(int i = 0 ; i < cl_list.size() ; i++){
                                        		Map map = (Map) cl_list.get(i);
                                        		%>
                                        		<tr>
		                                            <td><%=i+1 %></td>
		                                            <td><%=map.get("name") %></td>
		                                            <td><input type="checkbox" lay-skin="switch" lay-filter="switchTest" data-type="<%=map.get("id") %>" lay-text="开放|关闭" <%=(int)map.get("flag")==1?"checked":"" %>></td>
            ...
            $('#add_classs').click(function(){
                layer.open({
                    ...
                    ,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">'+
                        '<input type="text" name="title" required  lay-verify="required" placeholder="请输入班级名" autocomplete="off" class="layui-input" id="classs_name">'+
                        '</div>'
                    ,yes: function(index, layero){
                        var classs_name =  $("#classs_name").val();
                        if (classs_name != ''){
                            $.ajax({
                                async: false,
                                type:'POST',
